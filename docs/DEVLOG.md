...
354. **Realm.exe has its own icon now instead of MonoGame's default placeholder.** `Icon.ico`
     (`Realm.csproj`'s own `ApplicationIcon`) had shipped as MonoGame's stock orange "M" logo since
     the project's own template scaffolding -- never replaced with anything specific to this game.
     Requested via the `icon-design` skill; motif and palette (a sword, deep violet + gold with a
     magenta accent gem) chosen from a couple of quick options rather than guessed, since both
     materially change the result and only the user can pick a direction.

     New `Icon-master.svg` (a hand-authored 1024x1024 vector reference for future edits) and
     `Icon-master.png` (the actual 1024px raster the shipped `.ico` was generated from) added at the
     repo root alongside `Icon.ico` itself, matching where the old placeholder already lived rather
     than inventing a new assets folder. The PNG (and every embedded `.ico` frame) was rendered with
     Pillow at 4x supersampling for clean antialiased edges, not rasterized from the SVG directly --
     the SVG is a close hand-matched equivalent for whoever edits this next, not the literal source
     of the shipped bitmap. `Icon.ico` keeps the same 8-size set the placeholder already had (16/24/
     32/48/64/96/128/256), generated by handing Pillow's ICO writer the single supersampled master
     plus that `sizes` list (its own per-size resize, not a naive external append of pre-shrunk
     frames, which silently produced a broken single-16px-frame .ico on the first attempt here --
     caught by reopening the written file and checking `im.info["sizes"]` before finalizing).

     Design pass explicitly tested legibility across the whole shipped size range (16 through
     256px), not just the 1024px master: an early version added a soft magic glow behind the blade
     for atmosphere, but it muddied the gold blade's own edge into the violet background once
     downscaled to 16-32px (favicon/taskbar scale) -- dropped entirely in favor of a bolder blade
     taper and thicker dark outlines, which held a clean, recognizable sword-plus-gem silhouette
